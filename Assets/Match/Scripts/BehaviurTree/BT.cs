...
public class BT
{
	...
	public void setRootNode(BTNode node)
	{
		...
		DebugUtils.assert (true == existNode, "[BT->setRootNode]: node " + node._name + " does not exist");
	...
	{
		DebugUtils.assert (true == getExistNode(name), "[BT->getnodeByName]: node " + name + " does not exist!");
